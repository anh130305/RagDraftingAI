---
form_id: Form_05
form_type: "Công văn"
purpose: "Giao dịch, trao đổi công việc hành chính"

required_fields:
- TEN_CQ_CHU_QUAN
- TEN_CQ_BAN_HANH
- SO_CONG_VAN
- CHU_VIET_TAT_CQ_BAN_HANH
- CHU_VIET_TAT_DON_VI_SOAN_THAO
- DIA_DANH
- NGAY
- THANG
- NAM
- TRICH_YEU_CONG_VAN
- CO_QUAN_NHAN
- NOI_DUNG_CONG_VAN
- HO_VA_TEN_NGUOI_KY
---

{{TEN_CQ_CHU_QUAN}}  
**{{TEN_CQ_BAN_HANH}}**

---

Số: {{SO_CONG_VAN}}/{{CHU_VIET_TAT_CQ_BAN_HANH}}-{{CHU_VIET_TAT_DON_VI_SOAN_THAO}}  
V/v: {{TRICH_YEU_CONG_VAN}}

**CỘNG HÒA XÃ HỘI CHỦ NGHĨA VIỆT NAM**  
**Độc lập - Tự do - Hạnh phúc**

---

{{DIA_DANH}}, ngày {{NGAY}} tháng {{THANG}} năm {{NAM}}

**Kính gửi:**

- {{CO_QUAN_CA_NHAN_NHAN_1}}  
- {{CO_QUAN_CA_NHAN_NHAN_2}}

{{NOI_DUNG_CONG_VAN}}

---

**Nơi nhận:**

- Như trên  
- {{NOI_NHAN_KHAC}}  
- Lưu: VT, {{CHU_VIET_TAT_DON_VI_SOAN_THAO}} ({{SO_BAN_LUU}})

**{{QUYEN_HAN_CHUC_VU_NGUOI_KY}}**

*(Ký, đóng dấu)*

**{{HO_VA_TEN_NGUOI_KY}}**

---

{{DIA_CHI_CO_QUAN}}  
Email: {{THU_DIEN_TU}}  
Website: {{TRANG_THONG_TIN_DIEN_TU}}  
Điện thoại: {{SO_DIEN_THOAI}}  
Fax: {{SO_FAX}}